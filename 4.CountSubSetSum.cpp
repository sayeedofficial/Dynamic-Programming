...
int solve(int *arr, int n, int target_sum) {
    if (n == 0)
        return 0;
    if (target_sum == 0)
        return 1;
    ...
    return 1 + min(solve(arr, n, target_sum - arr[n - 1]), solve(arr, n - 1, target_sum));
}
...
int topDown(int *arr, int n, int target_sum) {
    if (memoArray[n - 1][target_sum] == -1) {
        ...
        if (arr[n - 1] > target_sum) {
            ...
            memoArray[n - 1][target_sum] = 1 + min(topDown(arr, n, target_sum - arr[n - 1]), topDown(arr, n - 1, target_sum));
        }
    }
